...
static void usage(void)
{
  ...
  puts(_("options:\n"
         ...
         "                separate attachment(s) from address(es) using the '--' argument.\n"
         ...
         "  -D            Dump all configuration variables as 'name=value' pair to stdout.\n"
         "  -D -S         Like -D, but hide the value of sensitive variables.\n"
         "  -d <level>    Log debugging output to file (by default to \"~/.neomuttdebug0\").\n"
         "                The level can range from 1–5 and affects verbosity.\n"
         "  -E            Edit draft (-H) or include (-i) file during message composition.\n"
         "  -e <command>  Specify a command to be run after initial program configuration."));
  puts(_("  -F <config>   Specify an alternative initialization file to read.\n"
         ...
         "  -n            Bypass loading of system-wide configuration file.\n"
         "  -p            Resume a prior postponed message, if any."));
  puts(_("  -Q <variable> Query a configuration variable after all configuration files and\n"
         "                commands (-e) have been processed and print its value to stdout.\n"
         ...
         "                remaining arguments as address even if they start with a hyphen."));
}
// clang-format on
